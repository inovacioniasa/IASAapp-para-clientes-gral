const CACHE_NAME = 'iasa-app-cache-v13';
const DATA_CACHE_NAME = 'iasa-app-data-cache-v13';
...
self.addEventListener('fetch', (event) => {
  ...
  // --- Navegación (páginas HTML) : network-first SIN cachear HTML ---
  const acceptsHtml = request.headers.get('accept') && request.headers.get('accept').includes('text/html');
  if (request.mode === 'navigate' || acceptsHtml) {
    event.respondWith(
      fetch(request)
        .then(networkResponse => {
          // NO cacheamos HTML para que los cambios lleguen al instante
          return networkResponse;
        })
        .catch(() => {
          // Si no hay red, devolver la página de fallback offline
  ...
  // EXCEPTO videos: los videos se descargan bajo demanda (lazy loading)
  // Detectar si es un video: por extensión, por type MIME, o por request.destination
  const isVideo = request.destination === 'video' ||
    /\.(mp4|webm|ogg|mov|m4v)(\?|$)/i.test(url.pathname) ||
    /firestore.*google.*news_uploads/i.test(url.href);

  if (isVideo) {
    // Para videos: NUNCA cachear, siempre network
    // No usar respondWith aquí, dejar que el navegador maneje la solicitud
    return;
  }
...
self.addEventListener('message', (event) => {
  ...
  // Permitir mensaje para forzar activación inmediata desde el cliente
  if (data === 'SKIP_WAITING' || data.type === 'SKIP_WAITING' || data.action === 'skipWaiting') {
    console.log('[Service Worker] Recibido SKIP_WAITING desde cliente. Ejecutando skipWaiting().');
    self.skipWaiting();
    return;
  }
